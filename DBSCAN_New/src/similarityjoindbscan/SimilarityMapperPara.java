...
public class SimilarityMapperPara extends Mapper<IntWritable,ArrayListDoubleWritable,KeyWritable2,ValueWritable> {

    @Override
    protected void map(IntWritable key, ArrayListDoubleWritable value, Context context)
                    throws IOException, InterruptedException {
//    	float epsilon = context.getConfiguration().getFloat("epsilon", 5.f);
        float epsilon = context.getConfiguration().getFloat(MainNew.PROPERTY.EPSILON.name(), 0);
